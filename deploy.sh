#!/bin/bash
# ============================================================
# Prism — Build & Deploy to Synology NAS
#
...
NAS_COMPOSE_DIR="/volume1/docker/prism"   # Synology path where compose lives
NAS_SMB_DIR="/mnt/k/prism"               # Fallback: Windows SMB mount
...
echo -e "${CYAN}${BOLD}  🔷 Prism — Deploy to Synology${RESET}"
if $DRY_RUN; then
...
if $HAS_SSH; then
  ...
  if $DRY_RUN; then
    ...
    info "  2. Project → prism → Stop → Start"
  fi
fi
