    ...
    steps.forEach((step, i) => {
        ...
        if (step.output) {
            ...
            if (step.outputImageRef)
                assistantMsg.images = [step.outputImageRef];
        ...
        // @ts-ignore
        if (step.output)
            result.text = step.output;
        // @ts-ignore
        if (step.outputImageRef)
            result.image = step.outputImageRef;
        // @ts-ignore
        ...
        // ── 5. Output Viewer ──
        {
            ...
            // @ts-ignore
            if (step.output)
                viewerResult.text = step.output;
            // @ts-ignore
            if (step.outputImageRef)
                viewerResult.image = step.outputImageRef;
            allNodes.push({
                id: viewerId,
                nodeType: "viewer",
                modality: null,
...
//# sourceMappingURL=WorkflowAssembler.js.map