...
async function init() {
    ...
    logger.info(`[InternalToolRegistry] Registered ${registry.size} internal tools: [${[...registry.keys()].join(", ")}]`);
}
// Kick off registration at module load
init().catch((error) => logger.error(`[InternalToolRegistry] Init failed: ${error.message}`));
...
//# sourceMappingURL=InternalToolRegistry.js.map