...
const googleProvider = {
    ...
    async captionImage(imageUrlOrBase64, prompt = 'Describe this image.', model = IMAGE2TEXT_DEFAULT_MODELS.google) {
    ...
    async generateImage(prompt, images = [], model = IMAGE2TEXT_MODELS.GEMINI_3_PRO) {
    ...
    async generateSpeech(text, voice = TEXT2SPEECH_DEFAULT_VOICES.google, options = {}) {
        ...
        try {
            ...
            const response = await getClient().models.generateContent({
                model: options.model || TEXT2SPEECH_DEFAULT_MODELS.google,
                contents: [
                    {
                        role: 'user',
                        parts: [{ text: options.prompt ? `${options.prompt}\n\n${text}` : text }],
                    },
                ],
                config,
            });
            ...
            if (candidates?.[0]?.content?.parts?.[0]?.inlineData) {
                ...
                if (inlineData.mimeType === 'audio/mpeg' || inlineData.mimeType === 'audio/mp3') {
                    return { stream: Readable.from(audioBuffer), contentType: 'audio/mpeg' };
                } else {
