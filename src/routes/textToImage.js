...
/**
 * POST /text-to-image
 * Body: { provider, model?, prompt, images?, conversationId?, userMessage? }
 * Response: { imageData (base64), mimeType, text?, provider, minioRef? }
 *
 * When conversationId is provided, the user message and generated image
    ...
    try {
        ...
        if (result.imageData) {
            try {
                ...
                logger.error(
                    `[text-to-image] MinIO upload failed: ${uploadErr.message}`,
                );
            }
        }
