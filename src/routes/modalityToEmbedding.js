...
router.post("/", async (req, res, next) => {
    ...
    try {
        ...
        if (!isMultimodal && text) {
            ...
            // Helper: extract base64 + mimeType from data URL or raw base64
        ...
        if (pricing?.inputPerMillion) {
            // Token count is not returned by the embedding API, so we estimate
            // based on rough heuristic: ~4 chars per token for text
            const approxTokens = text ? Math.ceil(text.length / 4) : 100;
            estimatedCost = (approxTokens / 1_000_000) * pricing.inputPerMillion;
        }

        logger.info(
            `[modality-to-embedding] ${providerName} model=${resolvedModel} — ` +
        ...
        RequestLogger.log({
            requestId,
            endpoint: "modality-to-embedding",
            project: req.project,
